    ...
    <div className="col-4 col-md-2 border-end px-0 bg-light flex-column d-flex">
      ...
      <ul id="channels-box"
        ...
        {channels.map((channel) => {
          ...
          const classChannel = cn('w-100', 'rounded-0', 'text-start', 'btn', {
            'btn-secondary': activeChannel === id,
            ...
            <li className="nav-item w-100" key={id}>
              {removable ? (
                <Dropdown as={ButtonGroup} className="d-flex dropdown btn-group">
                  <Button variant="outline" className={classChannel}
                  ...
                  <Dropdown.Menu style={{
                    ...
                    <Dropdown.Item
                      onClick={() => handleDelete(id)}>{t('modal.deleteBtn')}</Dropdown.Item>
                    <Dropdown.Item
                      onClick={() => handleRename(id)}>{t('modal.renameBtn')}</Dropdown.Item>
                  </Dropdown.Menu>
                </Dropdown>

              ) : (
                <button type="button" className={classChannel}
                        onClick={() => handleChannelClick(id)}>
                  <span
                    className="me-1"
                  >
